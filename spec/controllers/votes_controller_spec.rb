...
RSpec.describe VotesController, type: :controller do
  ...
  let(:my_vote)    { create(:votes) }

  context "guest" do
    describe "POST up_vote" do
      it "redirect the user to the sign in view" do
  ...
  context "signed in user" do
    ...
    describe "POST up_vote" do
      ...
      it "the users second vote deos not increase the number of votes" do
        post :up_vote, post_id: user_post.id
        votes = user_post.votes.count
        post :up_vote, post_id: user_post.id
      ...
      it "increases the sum of the post votes by one" do
    ...
    describe "POST down_vote" do
      it "the user first vote increases number of post votes by one" do
        votes = user_post.votes.count
        post :down_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes + 1)
      end

      it "the user second vote does not increase the number of the votes" do
        post :down_vote, post_id: user_post.id
        votes = user_post.votes.count
        post :down_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes)
